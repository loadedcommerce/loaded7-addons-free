...
class Loaded_Payments extends lC_Addon { // your addon must extend lC_Addon
  ...
  */
  public function install() {
    ...
    $lC_Database->simpleQuery("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Token Only Mode', 'ADDONS_PAYMENT_" . strtoupper($this->_code) . "_TOKEN_ONLY_MODE', '-1', 'Set to \'Yes\' for token only mode', '6', '0', 'lc_cfg_use_get_boolean_value', 'lc_cfg_set_boolean_value(array(1, -1))', now())");
  ...
  public function getKeys() {
    if (!isset($this->_keys)) {  
      $this->_keys = array('ADDONS_PAYMENT_' . strtoupper($this->_code) . '_STATUS',
                           ...
                           'ADDONS_PAYMENT_' . strtoupper($this->_code) . '_TOKEN_ONLY_MODE',
